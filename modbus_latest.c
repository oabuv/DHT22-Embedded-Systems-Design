...
//	delay_Ms(1000);
	...
	//unsigned char respond[7]={0x08,0x04,0x02,0x14,0x00,0xaa,0xee};
	//char buf[20] ="";
	unsigned char input[READ_LENGTH+1] = {0x08};
	int crc=0;
	char crc_high=0;
	char crc_low=0;
	int sensor_value = 0;
	...
	while (1)
	{
		...
		if (mFlag == 1)
		{
			...
			read_7_bytes_from_usartx(input);		// if the slave's addr is correct, then start to read the next 7 bytes
			...
			if(crc_high==input[7] && crc_low==input[6]){
				if(input[3] == 0x01){
					sensor_value = read_sensor(input[3]);
					// 0x08 0x04 0x00 0x01 0x00 0x01 0x60 0x93
					/*
				...
				else if(input[3] == 0x02){
					sensor_value = read_sensor(input[3]);
					// 0x08 0x04 0x00 0x02 0x00 0x01 0x90 0x93
					/*
...
void USART1_Init(void)
{
	RCC->APB2ENR|=0x4000; 	//set bit 15 (USART1 EN)
	RCC->AHBENR|=0x00000001; 	//enable GPIOA port clock bit 0 (GPIOA EN)
	GPIOA->AFR[1]=0x00000070;	//GPIOx_AFRH p.188,AF7 p.177. Because USART1 uses pa9 pa10 for tx and rx
	GPIOA->AFR[1]|=0x00000700;	//GPIOx_AFRH p.188,AF7 p.177
	GPIOA->MODER|=0x00080000; 	//MODER9=PA9(TX) to mode 10=alternate function mode. p184
	GPIOA->MODER|=0x00100000; 	//MODER10=PA10(RX) to mode 10=alternate function mode. p184
...
int read_sensor(int input_reg){
	...
	unsigned long long mask=0x80000000;		// mask will be shifted from right to left for 32 times
...
void respond_frame(int sensor_value){
	...
	//char respond[7]={0x08,0x04,0x02,0x14,0x00,0xaa,0xee};
	unsigned char respond[7]={0x08,0x04,0x02};
	int crc=0;

	/* Led On/Off for respond */
	GPIOA->MODER|=0x400; 			//GPIOA pin 5 to output. p184
	GPIOA->ODR|=0x20; //0010 0000 set bit 5. p186
	delay_Ms(500);
	GPIOA->ODR&=~0x20; //0000 0000 clear bit 5. p186
	...
	/* RE/DE control for MAX3485 from PA5 */
	// do not understand how RE/DE work yet!!

	/* Respond frame*/

	// divide the sensor value into 2 number of 2-bytes then put into respond frame
	respond[3] = sensor_value & 0xff;
	respond[4] = (sensor_value>>8);
	...
	// how to send the frame to master????
}
unsigned short int CRC16 (unsigned char *nData,unsigned short int wLength){
